    ...
    const sidebarContent = (
        ...
        <div className={`flex items-center h-16 px-4 border-b border-slate-100 ${collapsed ? 'justify-center' : 'justify-between'}`}>
            ...
            <div className="hidden lg:block px-3 py-4 border-t border-slate-100">
                <button
                    onClick={toggleCollapsed}
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-all"
                >
                    {collapsed ? (
                        <ChevronRight className="w-4 h-4" />
                    ) : (
